...
def country_picker_html(field_name='country_picker', selected='+967', picker_id='countryPicker'):
    ...
    <button type='button' class='countrySelect' onclick="openCountryPicker('{h(picker_id)}','{h(field_name)}')">
        <span id='{h(field_name)}_label'>{h(country_display(current))}</span><b> 1�7</b>
    </button>
...
def bad_request_error(e):
    return page("<div class='top'><a class='icon' href='/chats'> 1�7</a><b>طلب غير صحيح</b></div><div class='card'>انتهت صلاحية الصفحة أو الطلب غير آمن. ارجع وافتح الصفحة من جديد.</div>"), 400


@app.errorhandler(413)
def file_too_large(e):
    return page("<div class='top'><a class='icon' href='/chats'> 1�7</a><b>الملف كبير</b></div><div class='card'>حجم الملف أكبر من المسموح. اختر ملفًا أصغر.</div>"), 413


@app.errorhandler(500)
def internal_error(e):
    return page("<div class='top'><a class='icon' href='/chats'> 1�7</a><b>حدث خطأ</b></div><div class='card'>حدث خطأ داخلي. أعد تشغيل الصفحة، وإذا تكرر الخطأ أرسل آخر سطر من Termux.</div>"), 500
...
def process_due_reminders(user_id):
    try:
        ...
        for r in rows:
            text = '⏄1�7 تذكير برسالة: ' + ((r['body'] or 'ملف مرفق')[:70])
...
function showFileName(input){{
  ...
  if(input.files&&input.files[0]){{
    ...
    let icon=kind==='image'?'🖼︄1�7':kind==='video'?'🎬':kind==='audio'?'🎵':'📄';
...
def nav(active):
    ...
    items = [("chats","💬","المحادثات",''),("statuses","◄1�7","الحالات",''),("calls","📞","المكالمات",''),("notifications","🔔","الإشعارات",badge),("me","👤","حسابي",'')]
...
def verify_email():
    ...
    info_box = ''
    if flash:
        info_box = f"<div class='card' style='border-color:#22c55e;color:#bbf7d0'>✄1�7 {h(flash)}</div>"
    if send_error:
        info_box = f"<div class='card' style='border-color:#ef4444;color:#fecaca'>⚠️ {h(send_error)}</div>"
    email = h(u['email'])
    return page(f"""
    <div class='top'><a class='icon' href='/login'> 1�7</a><b>تحقق البريد</b></div>
...
def logout():
    ...
    avatar = avatar_url(u)
    return page(f"""
    <div class='top'><a class='icon' href='/me'> 1�7</a><b>تسجيل الخروج</b></div>
...
def chats():
    ...
    if 'service_chat_enabled' in u.keys() and not u['service_chat_enabled']:
        return page("<div class='top'><a class='icon' href='/services'> 1�7</a><b>المحادثات متوقفة</b></div><div class='card'>خدمة المحادثات متوقفة من الإعدادات. <a class='btn' href='/services'>تشغيلها الآن</a></div>")
    ...
    <div class='chatHome'>
        <div class='chatHomeTop'>
            <a class='chatTopIcon' href='/me'>☄1�7</a>
            <div class='chatLogo'>واصل</div>
            <button class='chatTopIcon searchOnlyBtn' type='button' onclick="toggleChatSearch()">⌄1�7</button>
        </div>
        ...
        <a class='chatFab' href='/new_contact'>＄1�7<small>محادثة جديدة</small></a>
    </div>
    <script>
...
def archived():
    ...
    return page(f"<div class='top'><a class='icon' href='/chats'> 1�7</a><b>الأرشيف</b></div><div class='list'>{rows or '<div class=card>لا توجد محادثات مؤرشفة</div>'}</div>")
...
def new_contact():
        ...
        if found and found['id'] != u['id'] and found['id'] not in seen_ids:
            ...
            <div class='card'>
                ...
                <form method='post' action='/address_book/add' style='margin-top:12px'>
                    ...
                    <button class='btn' style='width:100%'>➄1�7 حفظ ومراسلة</button>
                </form>
            </div>
            """
    ...
    body = f"""
    <div class='top'><a href='/chats' class='icon'> 1�7</a><b>إنشاء جديد</b></div>
    <form class='search' method='get'>
        <input name='q' value='{h(q)}' placeholder='بحث باسم محفوظ، أو رقم/بريد'>
    </form>
    {'<div class=card><p class=danger>'+h(msg)+'</p></div>' if msg else ''}
    <div class='card'>
        <a class='btn' style='width:100%;display:block;text-align:center' href='/address_book/add'>➄1�7 إضافة جديد</a>
...
def address_book_add():
    ...
    return page(f"""
    <div class='top'><a class='icon' href='/new_contact'> 1�7</a><b>إضافة جديد</b></div>
    <form class='card' method='post'>
...
def address_book_chat(book_id):
    ...
    if not linked_id:
        return page(f"<div class='top'><a class='icon' href='/new_contact'> 1�7</a><b>غير متاح</b></div><div class='card'>هذا الشخص محفوظ عندك لكنه لم يدخل المنصة بعد.</div>")
...
def chat(peer_id):
    ...
    for m in msgs:
        ...
        ticks = ('✓✓' if m['is_read'] else '✄1�7') if cls=='me' else ''
        edited = ' · معدلة' if m['edited_at'] else ''
        star = '⭄1�7 ' if m['starred'] else ''
  ...
  <div class='sheet' onclick='event.stopPropagation()'>
    ...
    <div class='actions'>
      ...
      <div class='actionGroup'>
        ...
        <button class='yellow' onclick="location.href='/message/{mid}/star'"><span class='ico'>⭄1�7</span><span>حفظ / إلغاء الحفظ</span></button>
        ...
        <button class='orange' onclick="location.href='/message/{mid}/reminder'"><span class='ico'>⏄1�7</span><span>تذكير</span></button>
      </div>
      <div class='actionGroup'>
        <button class='orange' onclick="location.href='/message/{mid}/edit'"><span class='ico'>✏️</span><span>تعديل</span></button>
        <button class='red' onclick="location.href='/message/{mid}/delete_me'"><span class='ico'>🗑︄1�7</span><span>حذف لدي</span></button>
...
def forward_message(mid):
    ...
    return page(f"<div class='top'><a class='icon' href='/chat/{old_peer}'> 1�7</a><b>تحويل الرسالة</b></div><form class='card' method='post'><p class='muted'>اختر جهة اتصال لتحويل الرسالة إليها.</p>{rows or '<div class=muted>لا توجد جهات اتصال متاحة.</div>'}<br><button class='btn'>تحويل</button></form>")
...
def edit_message(mid):
    ...
    body = f"<div class='top'><a class='icon' href='/chat/{m['receiver_id']}'> 1�7</a><b>تعديل الرسالة</b></div><form class='card' method='post'><div class='input'><textarea name='body' rows='5'>{m['body'] or ''}</textarea></div><button class='btn'>حفظ التعديل</button></form>"
    return page(body)
...
def message_info(mid):
    ...
    if not m or (m['sender_id'] != session['user_id'] and m['receiver_id'] != session['user_id']):
        return redirect('/chats')
    peer = m['receiver_id'] if m['sender_id'] == session['user_id'] else m['sender_id']
    body = f"<div class='top'><a class='icon' href='/chat/{peer}'> 1�7</a><b>معلومات الرسالة</b></div><div class='card'><p>الوقت: {m['created_at']}</p><p>محفوظة: {'نعم' if m['starred'] else 'لا'}</p><p>التفاعل: {m['reaction'] or 'لا يوجد'}</p><p>تمت القراءة: {'نعم' if m['is_read'] else 'لا'}</p><p>وقت القراءة: {m['read_at'] or 'لم تقرأ بعد'}</p><p>آخر تعديل: {m['edited_at'] or 'لا يوجد'}</p><p>يوجد ملف: {'نعم' if m['file_name'] else 'لا'}</p></div>"
    return page(body)
...
def chat_audio(peer_id):
    ...
    voice_body = caption or '🎙︄1�7 تسجيل صوتي'
...
def starred_messages():
    ...
    for m in rows:
        ...
        cards += f"<a class='chatitem' href='/chat/{peer}'><div class='grow'><div class='name'>⭄1�7 {who}</div><div class='last'>{m['body'] or 'ملف مرفق'} · {m['created_at']}</div></div></a>"
    return page(f"<div class='top'><a class='icon' href='/me'> 1�7</a><b>الرسائل المميزة</b></div><div class='list'>{cards or '<div class=card>لا توجد رسائل مميزة</div>'}</div>")
...
def profile(uid):
    ...
    body = f"""
    <div class='top'><a class='icon' href='{back}'> 1�7</a><b>الملف الشخصي</b></div>
...
def status_reactions(sid):
    ...
    return page(f"<div class='top'><a class='icon backHistory' href='/statuses' onclick='return goBackOne()'> 1�7</a><b>تفاعلات الحالة</b></div><div class='card'><b>عدد التفاعلات: {len(rows_db)}</b></div><div class='statusSubList'>{rows or '<div class=card>لا توجد تفاعلات حتى الآن</div>'}</div>")
...
def statuses():
    ...
    if 'service_status_enabled' in u.keys() and not u['service_status_enabled']:
        return page("<div class='top'><a class='icon' href='/services'> 1�7</a><b>الحالات متوقفة</b></div><div class='card'>خدمة الحالات متوقفة من الإعدادات. <a class='btn' href='/services'>تشغيلها الآن</a></div>")
    ...
    body = f"""
    <div class='top'><a class='icon' href='/chats'> 1�7</a><b style='margin:auto'>الحالات</b><a class='icon green' href='#newStatus'>＄1�7</a></div>
...
def view_status(sid):
    ...
    return page(f"""
    <div class='top'><a class='icon backHistory' href='/statuses' onclick='return goBackOne()'> 1�7</a><img class='avatar' src='{av}'><div><b>{h(st['name'])}</b><div class='muted'>{h(elapsed)}</div></div></div>
...
def status_viewers(sid):
    ...
    return page(f"<div class='top'><a class='icon backHistory' href='/statuses' onclick='return goBackOne()'> 1�7</a><b>مشاهدات الحالة</b></div><div class='card'><b>عدد المشاهدات: {len(viewers)}</b></div><div class='list'>{rows or '<div class=card>لا توجد مشاهدات حتى الآن</div>'}</div>")
...
def status_replies(sid):
    ...
    return page(f"<div class='top'><a class='icon backHistory' href='/statuses' onclick='return goBackOne()'> 1�7</a><b>ردود الحالة</b></div><div class='card'><b>عدد الردود: {len(replies)}</b><div class='muted'>الردود هنا خاصة داخل الحالة، وليست رسائل محادثة.</div></div><div class='statusReplyListPage'>{rows or empty}</div>")
...
def calls():
    ...
    if 'service_calls_enabled' in u.keys() and not u['service_calls_enabled']:
        return page("<div class='top'><a class='icon' href='/services'> 1�7</a><b>المكالمات متوقفة</b></div><div class='card'>خدمة المكالمات متوقفة من الإعدادات. <a class='btn' href='/services'>تشغيلها الآن</a></div>")
    rows_db = db().execute("""
    ...
    return page(f"<div class='top'><a class='icon' href='/chats'> 1�7</a><b style='margin:auto'>المكالمات</b></div><div class='list'>{rows or '<div class=card>لا توجد مكالمات</div>'}</div>{nav('calls')}")
...
def call(peer, typ):
    ...
    if is_blocked_between(u['id'], peer):
        return page(f"<div class='top'><a class='icon' href='/chat/{peer}'> 1�7</a><b>المكالمة ممنوعة</b></div><div class='card'>لا يمكن بدء المكالمة لأن الحظر مفعل بينكما.</div>")
...
def incoming_call(call_id):
    ...
    icon = '📹' if c['call_type'] == 'video' else '📞'
    return page(f"<div class='top'><a class='icon' href='/calls'> 1�7</a><b>مكالمة واردة</b></div><div class='card' style='text-align:center;margin-top:60px'><img class='avatar' style='width:130px;height:130px' src='{avatar_url(p)}'><h2>{p['name']}</h2><p class='status'>{icon} {'مكالمة فيديو واردة' if c['call_type']=='video' else 'مكالمة صوتية واردة'}</p><div class='callBtns'><a class='btn' href='/call_accept/{call_id}'>قبول</a><a class='btn' style='background:#ef4444' href='/call_decline/{call_id}'>رفض</a><a class='btn gray' href='/calls'>رجوع</a></div></div>")
...
def call_room(call_id):
    ...
    if c['status'] in ('مرفوضة', 'منتهية', 'فائتة'):
        return page(f"<div class='top'><a class='icon' href='/calls'> 1�7</a><b>تفاصيل المكالمة</b></div><div class='card'><h3>{p['name']}</h3><p>الحالة: {c['status']}</p><p>النوع: {'فيديو' if c['call_type']=='video' else 'صوت'}</p><p>الوقت: {c['created_at']}</p><a class='btn' href='/call/{peer_id}/{c['call_type']}'>إعادة الاتصال</a></div>")
...
function toggleMic(){{micOn=!micOn; localStream?.getAudioTracks().forEach(t=>t.enabled=micOn); document.getElementById('micBtn').innerText=micOn?'🎙︄1�7 كتم':'🔇 تشغيل'}}
...
startCall();
    ...
    body = f"<div class='top'><a class='icon' href='/chat/{peer_id}'> 1�7</a><img class='avatar' src='{avatar_url(p)}'><div class='header-name'><b>{p['name']}</b><div class='status' id='callStatus'>{'يرن الآن...' if c['status']=='رنين' else 'بدء المكالمة...'}</div></div></div><div style='position:relative;padding:16px'>{video_html}<div class='card'><div class='callBtns'><button class='btn gray' id='micBtn' onclick='toggleMic()'>🎙︄1�7 كتم</button><button class='btn gray' id='camBtn' onclick='toggleCam()'>📹 إيقاف</button><button class='btn' style='background:#ef4444' onclick='endCall()'>إنهاء</button></div><p class='muted'>المرحلة 6: الاتصال صار فيه قبول/رفض وحالة رنين ومدة للمكالمة.</p></div></div>{script}"
    return page(body)
...
def notifications():
    ...
    html = ''
    icons = {'message':'💬', 'status':'◄1�7', 'call':'📞', 'reminder':'⏄1�7', 'general':'🔔'}
    for n in rows:
    ...
    return page(f"<div class='top'><a class='icon' href='/chats'> 1�7</a><b style='margin:auto'>الإشعارات</b></div>{tools}<div class='list'>{html or '<div class=card>لا توجد إشعارات</div>'}</div>{nav('notifications')}")
...
def service_card(icon, title, desc, status, link):
    st = '✄1�7 ' + status if 'مفعلة' in status or 'جاهزة' in status else '⚠️ ' + status
...
def settings_home():
    u=current_user()
    body=f"""<div class='top'><a class='icon' href='/me'> 1�7</a><b>الإعدادات</b></div>
    ...
    <div class='card'>
      ...
      <a class='chatitem' href='/security_check'><div class='icon'>🛡︄1�7</div><div class='grow'><div class='name'>فحص الأمان والاستقرار</div><div class='last'>فحص سريع للجداول والخدمات</div></div></a>
    </div>{nav('me')}"""
    return page(body)
...
def account_settings():
    ...
    body=f"""<div class='top'><a class='icon' href='/settings'> 1�7</a><b>إعدادات الحساب</b></div>
...
def appearance():
    ...
    body=f"""<div class='top'><a class='icon' href='/settings'> 1�7</a><b>المظهر</b></div>
...
def storage_settings():
    ...
    body=f"""<div class='top'><a class='icon' href='/settings'> 1�7</a><b>التخزين والملفات</b></div>
    ...
    <label class='chatitem'><input type='checkbox' name='save_media_gallery' {chk(u['save_media_gallery'])}> 🖼︄1�7 حفظ الوسائط في المعرض</label>
    <button class='btn'>حفظ</button></form>"""
    return page(body)
...
def services_settings():
    ...
    def chk(v): return 'checked' if v else ''
    body=f"""<div class='top'><a class='icon' href='/settings'> 1�7</a><b>الخدمات الموجودة</b></div>
    ...
    {service_card('◄1�7','الحالات','حالاتي المنشورة · '+str(counts['statuses'])+' حالة',onoff(u['service_status_enabled']),'/statuses')}
    ...
    <label class='chatitem'><input type='checkbox' name='service_status_enabled' {chk(u['service_status_enabled'])}> ◄1�7 تشغيل الحالات</label>
...
def security_check():
    ...
    rows=''.join([f"<div class='chatitem'><div class='icon'>{'✄1�7' if ok else '⚠️'}</div><div class='grow'><div class='name'>{table}</div><div class='last'>{'جاهز' if ok else 'ناقص: '+miss}</div></div></div>" for table,ok,miss in checks])
    return page(f"<div class='top'><a class='icon' href='/settings'> 1�7</a><b>فحص الأمان والاستقرار</b></div><div class='card'>{rows}</div>")
...
def account_health(u):
    ...
    rows=''.join([f"<div class='chatitem'><div class='icon'>{'✄1�7' if v else '⚠️'}</div><div class='grow'><div class='name'>{h(k)}</div><div class='last'>{'مكتمل' if v else 'يحتاج ضبط'}</div></div></div>" for k,v in checks])
...
if __name__ == '__main__':
    # �1�2�1�1�1�4�1�8�1�1 �1�6�1�9�1�7�1�7�1�1 �1�9�1�8�1�0�1�4�1�9�1�0�1�9�1�2 �1�2�1�8�1�6�1�9�1�8�1�4�1�9�1�5 �1�7�1�8�1�3 �1�1�1�4�1�9�1�5�1�9 �1�9�1�4�1�0�1�7�1�9
    init_db()
    
    # �1�6�1�9�1�9�1�3�1�1 �1�9�1�8�1�9�1�0�1�5�1�8 �1�9�1�8�1�7�1�4�1�0�1�9�1�9�1�4�1�7�1�4 �1�9�1�0 �1�1�1�4�1�9�1�5�1�9 �1�9�1�4�1�0�1�7�1�9�1�2 �1�2�1�7�1�8�1�9 �1�8�1�9 �1�4�1�4�1�7�1�1 �1�4�1�6�1�2�1�9�1�9 5000 �1�2�1�8�1�6�1�9�1�8�1�4�1�9�1�5
    port = int(os.environ.get('PORT', 5000))
    
    print(f'�0�4 �1�2�1�5�1�0�1�4�1�6 �1�2�1�9�1�3�1�8 �1�2�1�9�1�2 �1�4�1�0�1�7�1�5 �1�9�1�8�1�7�1�9�1�8 �1�9�1�8�1�4�1�0 �1�7�1�8�1�3 �1�9�1�8�1�9�1�0�1�5�1�8: {port}')
    
    # �1�2�1�2�1�8�1�4�1�8 �1�9�1�8�1�2�1�5�1�0�1�4�1�6 �1�0�1�9�1�8�1�9�1�0�1�5�1�8 �1�9�1�8�1�3�1�5�1�4�1�5
